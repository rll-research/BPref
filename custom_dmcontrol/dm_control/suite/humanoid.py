...
@SUITE.add('benchmarking')
def human(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns the Stand task."""
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = Human(move_speed=0, pure_state=False, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
class Human(base.Task):
  ...
  def __init__(self, move_speed, pure_state, random=None):
    ...
    self._move_speed = move_speed
    self._pure_state = pure_state
    super(Human, self).__init__(random=random)
  ...
  def initialize_episode(self, physics):
    ...
    while penetrating:
      ...
      physics.after_reset()
      penetrating = physics.data.ncon > 0
    super(Human, self).initialize_episode(physics)
